...
import { Car, Gauge, Wrench, QrCode, CheckCircle2, ShieldCheck, ShieldAlert, AlertTriangle, Camera, Clock, CircleDollarSign, Bell, Timer, ExternalLink, Shield, Battery } from "lucide-react";
      ...
      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        {data.ev?.isElectric && (
          <Box title="Electric Vehicle" icon={<Battery className="w-4 h-4 text-slate-500" />}>
            ...
            {(data.ev?.provenance || data.ev?.smartcarCompatible || data.ev?.capabilities?.obd_ev_pids) && (
              <div className="text-xs text-slate-500 mt-3 space-y-1">
                <div className="flex items-center flex-wrap gap-1">
                  <span>Detection:</span>
                  <span>{data.ev?.provenance?.detection === 'vin_heuristic' ? 'VIN analysis' : 'Manual'}</span>
                  {typeof data.ev?.provenance?.detectionConfidence === 'number' && (
                    <span>({Math.round(data.ev.provenance!.detectionConfidence * 100)}% confidence)</span>
                  )}
                </div>
