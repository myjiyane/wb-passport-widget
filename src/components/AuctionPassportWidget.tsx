...
// Add props for navigation callback
...
export default function AuctionPassportWidget({ data, onNavigateToVerification }: AuctionPassportWidgetProps) {
  ...
  const handleVerificationClick = () => {
    if (onNavigateToVerification) {
      ...
      // Fallback: navigate to verification URL
      ...
      <div className="mt-4 grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <Box title="Integrity & Seal" icon={<ShieldCheck className="w-4 h-4 text-slate-500" />}>
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            <button
              onClick={handleVerificationClick}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-teal-600 text-white px-4 py-2 text-sm font-medium shadow-sm hover:bg-teal-700"
            >
          ...
          {/* Verification badge - shown when verified */}
          {data.seal?.valid && (
